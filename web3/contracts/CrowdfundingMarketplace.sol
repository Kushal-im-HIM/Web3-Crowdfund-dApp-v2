...
contract CrowdfundingMarketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    function createCampaign(
        ...
        require(
            ...
            "Insufficient campaign creation fee"
        );
        ...
        nonReentrant
    {
        ...
        // FIX (Issue #1): Enforce hard cap at campaign targetAmount.
        // Previously, msg.value was accepted unconditionally, allowing campaigns to be
        // over-funded beyond their target. Now we calculate the remaining allowance and
        // cap the accepted amount. Any excess ETH is refunded to the sender atomically.
        uint256 remainingAllowance = campaign.targetAmount >
            campaign.raisedAmount
            ? campaign.targetAmount - campaign.raisedAmount
            : 0;
        require(
            remainingAllowance > 0,
            "Campaign funding target already reached"
        );

        // Cap the accepted contribution; refund whatever exceeds the remaining allowance.
        uint256 acceptedAmount = msg.value > remainingAllowance
            ? remainingAllowance
            : msg.value;
        uint256 refundAmount = msg.value - acceptedAmount;

        // --- original contribution accounting (unchanged, just uses acceptedAmount) ---
        // Original code used msg.value directly — replaced with acceptedAmount.
        ...
        // CHANGED: was contributions[_campaignId][msg.sender] += msg.value;
        contributions[_campaignId][msg.sender] += acceptedAmount;
        // CHANGED: was campaign.raisedAmount += msg.value;
        campaign.raisedAmount += acceptedAmount;

        campaignContributions[_campaignId].push(
            Contribution({
                contributor: msg.sender,
                // CHANGED: was msg.value
                amount: acceptedAmount,
                timestamp: block.timestamp
            })
        );

        // Refund excess to contributor (safe — reentrancy guard is active)
        if (refundAmount > 0) {
            payable(msg.sender).transfer(refundAmount);
        }

        // CHANGED: was emit ContributionMade(_campaignId, msg.sender, msg.value);
        emit ContributionMade(_campaignId, msg.sender, acceptedAmount);
    ...
    function getActiveCampaigns(
        ...
        for (
            uint256 i = 1;
            i <= campaignCounter && resultIndex < returnCount;
            i++
        ) {
